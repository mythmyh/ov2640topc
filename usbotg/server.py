...
def decode_array(number):
    length = math.ceil(number/255)
    decode_array = [255 if x< length-1 else number % 255 if x == length-1 else 0 for x in range(4)]
    if number % 255 == 0:
        decode_array[int(length)-1] = 255
    return decode_array

def receive_period(_clientsocket, _counter_set, _start, _end):
    global per_mss,buf
    _counter = _start
    number =_start+circle_times+1
    deco = decode_array(number)

    _clientsocket.send(bytes(deco))
    while _counter < _end:
...
if __name__ == '__main__':
    ...
    # 绑定端口号
    serversocket.bind(("192.168.1.26", port))
    ...
    while True:
        ...
        _start = time.time()
        deco = decode_array(1000)
        #clientsocket.send(bytes(deco))
        ...
        #if lasttime > 1:
        print(filename, " right", str(len(buf)), "use time ", str(lasttime))
            #print(filename)
